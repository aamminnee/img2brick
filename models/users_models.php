...
class UsersModel {
    ...
    public function getStatusById($id_user) {
        $request = mysqli_prepare($this->conn, "SELECT etat FROM user WHERE id_user = ?");
    ...
    public function addUser($email, $username, $password) {
        ...
        $request = mysqli_prepare($this->conn, "INSERT INTO user (email, username, etat, mdp) VALUES (?, ?, 'invalide', ?)");
    ...
    public function activateUser($id_user) {
        $request = mysqli_prepare($this->conn, "UPDATE user SET etat = 'valide' WHERE id_user = ?");
        mysqli_stmt_bind_param($request, "i", $id_user);
        return mysqli_stmt_execute($request);
    }
}
