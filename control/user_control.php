...
class UserController {
    ...
    // logic for the reset password
    public function reset() { 
        if (isset($_POST['password']) && isset($_POST['password2'])) { 
            if ($_POST['password'] === $_POST['password2']) {
                $this->user_model->resetPassword($_SESSION['id_user'], $password);
            } else {
                echo "Erreur : mot de passe différent";
            }
        } else {
            echo "Erreur ; veuillez remplir les champs du formulaire";
        }
    }
